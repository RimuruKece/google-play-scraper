...
const MAPPINGS = {
  ...
  developerLegalAddress: {
    ...
    fun: (searchArray: ScrapedData) => {
      return R.path([4, 2, 0], searchArray)?.toString().replace(/\n/g, ', ');
    }
  },
  ...
  categories: {
    path: ['ds:5', 1, 2],
    fun: (searchArray: ScrapedData) => {
      const categories = helper.extractCategories(R.path([118], searchArray) as ScrapedData);
      if (categories.length === 0) {
        categories.push({
          name: R.path([79, 0, 0, 0], searchArray) as string,
          id: R.path([79, 0, 0, 2], searchArray) as string | null
        });
      }
      return categories;
    }
