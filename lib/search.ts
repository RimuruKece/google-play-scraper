...
async function processFirstPage (html: ScrapedData, opts: SearchOptions, savedApps: AppListItem[], mappings: Mappings): Promise<AppListItem[]> {
  ...
  const sections = R.path(mappings.sections, html) as unknown[] || [];
  ...
  for (const section of sections) {
    ...
    if (Array.isArray(apps) && (apps as unknown[]).length > 0) {
      appsSection = apps;
      tokenValue = R.path(SECTIONS_MAPPING.token, section) as string | null;
      break;
    }
  }
  ...
  const exactMatchData = R.path(mappings.app, html) as ScrapedData;
  if (exactMatchData) {
...
async function search (appData: (opts: Record<string, unknown>) => Promise<unknown>, opts: SearchOptions): Promise<AppListItem[]> {
  ...
  const results = await initialRequest(opts as SearchOptions);

  if (opts.fullDetail) {
    return Promise.all(
      results.map((app: AppListItem) => appData({ ...opts, appId: app.appId }) as Promise<unknown>)
