...
function flatMapPermissions (permission: ScrapedData): PermissionItem[] {
  const input = R.path(MAPPINGS.permissions, permission) as ScrapedData[] | undefined;
  ...
  return R.map(scriptData.extractor(mappings), input) as unknown as PermissionItem[];
}
