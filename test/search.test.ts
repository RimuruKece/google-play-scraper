...
describe('Search method', () => {
  ...
  describe('additional properties', () => {
    it('should fetch a valid application list with developer property', () => {
      return gplay
        ...
        .then((apps) => apps.map((app: any) => assert.isString(app.developer)));
    });
    ...
    it('should fetch a valid application list with developerId property', () => {
      return gplay
        ...
        .then((apps) => apps.map((app: any) => assert.isString(app.developerId)));
    });
  });
  ...
  it('should search for pre register with fullDetail', () =>
    gplay
      ...
      .then((apps) => apps.map(assertValidApp))).timeout(15 * 1000);
  ...
  describe('more results mapping', () => {
    it('should return few netflix apps', () => {
      return gplay.search({ term: 'netflix' }).then((apps) => {
        ...
        apps.map(assertValidApp);
        assert.isTrue(
          apps.some((app: any) => app.developer.toLowerCase().includes('netflix')),
    ...
    it('should return few google mail apps', () => {
      return gplay.search({ term: 'gmail' }).then((apps) => {
        ...
        apps.map(assertValidApp);
        assert.isTrue(
          apps.some((app: any) => app.developer === 'Google LLC'),
    ...
    it('should include the exact match app when search term is an exact app name', () => {
      return gplay.search({ term: 'Google Chrome' }).then((apps) => {
        ...
        apps.map(assertValidApp);
        assert.isTrue(
          apps.some((app: any) => app.appId === 'com.android.chrome'),
