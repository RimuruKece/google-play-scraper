...
function descriptionHtmlLocalized (searchArray: ScrapedData): string | undefined {
  const descriptionTranslation = path([12, 0, 0, 1], searchArray) as string | undefined;
  const descriptionOriginal = path([72, 0, 1], searchArray) as string | undefined;
...
function extractComments (data: ScrapedData): string[] {
  ...
  for (const dsKey of ['ds:8', 'ds:9']) {
    if (path([dsKey, 0, 0, 1, 0], data) && path([dsKey, 0, 0, 10], data) && path([dsKey, 0, 0, 5, 0], data)) {
      comments = (path([dsKey, 0], data) as ScrapedData[]) || [];
      break;
    }
  }
...
function extractFeatures (featuresArray: ScrapedData): Feature[] {
  ...
  return features.map((feature) => ({
    ...
    description: path([1, 0, 0, 1], feature) as string
  }));
}
