...
function validate (opts: ReviewsOptions): void {
  ...
  if (opts.sort && !(Object.values(constants.sort) as number[]).includes(opts.sort)) {
...
async function processReviewsAndGetNextPage (html: ScrapedData, opts: ReviewsOptions, savedReviews: ReviewItem[]): Promise<ReviewsResult> {
  ...
  const { appId, paginate, num = 150 } = processAndRecurOptions;
  const parsedHtml = typeof html === 'string'
  ...
  const token = path(REQUEST_MAPPINGS.token, parsedHtml) as string | null;
...
async function makeReviewsRequest (opts: ReviewsOptions, savedReviews: ReviewItem[], nextToken: string): Promise<ReviewsResult> {
  ...
  const resolvedNum = num ?? 150;
  const body = getBodyForRequests({
    appId: appId ?? '',
    sort: sort ?? constants.sort.NEWEST,
    withToken: nextToken,
    requestType
  });
  ...
  return (data === null)
    ? formatReviewsResponse({ reviews: savedReviews, token: null, num: resolvedNum })
...
function extract (root: (string | number)[], data: ScrapedData, appId: string): ReviewItem[] {
  const input = path(root, data) as ScrapedData[] | undefined;
  const mappings = {
    ...REVIEWS_BASE_MAPPINGS,
    url: { path: [0], fun: (reviewId: string) => `${BASE_URL}/store/apps/details?id=${appId}&reviewId=${reviewId}` }
  };
  return (input || []).map(scriptData.extractor(mappings)) as unknown as ReviewItem[];
}
